...
export function RunTrace({ events }: { events: LiveEvent[] }) {
  ...
  return (
    <div ref={scrollRef} className="h-96 overflow-y-auto bg-slate-950 px-4 py-3 font-mono text-xs leading-relaxed">
      {lines.length === 0 ? (
        ...
        lines.map((l, i) => (
          <div key={i} className={KIND_COLOR[l.kind]}>
            ...
            {l.kind === "error" && l.stack ? (
              <pre className="mt-1 whitespace-pre-wrap break-words rounded border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-rose-300">
                {l.stack}
              </pre>
            ) : null}
          </div>
